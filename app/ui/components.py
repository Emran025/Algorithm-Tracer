...
def playback_controls(on_play: Callable,on_back_to_start: Callable, on_pause: Callable, on_step_forward: Callable, on_step_back: Callable, on_seek: Callable, current_step_index: int, total_steps: int, is_playing: bool):
    ...
    with col2:
        if is_playing and total_steps - 1 > current_step_index :
            ...
            if st.button(" ◀ Play "):
                if total_steps - 1 == current_step_index:
                    on_back_to_start()
                on_play()
    with col3:
...
def data_input_form(algorithm_type: str , algorithm_name : str) -> Dict[str, Any]:
    ...
    if algorithm_type == "array":
        ...
        if input_method == "Manual Input":
            ...
            if "generated_array" in st.session_state and st.session_state.generated_array is not None:
...
def algorithm_example_analysis_panel(algorithm_name: str , n:int , arr: list, target: int  , v: int, e: int):
    """Displays a panel with time/space complexity and pedagogical notes.

    Args:
        algorithm_name (str): The name of the algorithm.
    """
    st.subheader(f"Example Analysis:")
    analysis_data = {
        "Merge Sort": {
            "Time Complexity": merge_sort_time_complexity(n),
            "Space Complexity": merge_sort_space_complexity(n),
        },
        "Quick Sort": {
            "Time Complexity": quick_sort_time_complexity(n),
            "Space Complexity": quick_sort_space_complexity(n),
        },
        "Linear Search": {
            "Time Complexity": linear_search_time_complexity(arr, target),
            "Space Complexity": linear_search_space_complexity(arr, target),
        },
        "Kruskal (MST)": {
            "Time Complexity": kruskal_time_complexity(v, e),
            "Space Complexity": kruskal_space_complexity(v, e),
        },
        "Dijkstra (SSSP)": {
            "Time Complexity": dijkstra_time_complexity(v, e),
            "Space Complexity": dijkstra_space_complexity(v, e),
        }
    }

    algo_info = analysis_data.get(
        algorithm_name,
        {"Time Complexity": "N/A", "Space Complexity": "N/A"}
    )

    st.markdown(f"{algo_info['Time Complexity']}")
    st.markdown(f"{algo_info['Space Complexity']}")


# 1) Merge Sort Complexity
def merge_sort_time_complexity(n: int) -> str:
    if n <= 0:
        return "**Time Complexity** = N/A"
    time = n * math.log2(n)
    return f"**Time Complexity** ≈ {time:.2f}"

def merge_sort_space_complexity(n: int) -> str:
    space = n
    return f"**Space Complexity** = {space}"

# 2) Quick Sort Complexity


def quick_sort_time_complexity(n: int) -> str:
    if n <= 0:
        return "**Time Complexity** = N/A"
    worst = n**2
    average = n * math.log2(n)
    return f"**worst-case** = {worst} **average** ≈ {average:.2f}"

def quick_sort_space_complexity(n: int) -> str:
    if n <= 0:
        return "**Space Complexity** = N/A"
    space = math.log2(n)
    return f"**Space Complexity** ≈ {space:.2f}"

# 3) Linear Search
def linear_search_time_complexity(arr: list, target: int) -> str:
    try:
        return f"**Time Complexity**  = {len(arr)}"
    except Exception:
        # If arr is not iterable or not list of values, return a generic message
        return f"**Time Complexity**  = N/A"

def linear_search_space_complexity(arr: list, target: int) -> str:
    try:
        for i, val in enumerate(arr):
            if val == target:
                return f"Target {target} **find** in {i}"
    except Exception:
        # If arr is not iterable or not list of values, return a generic message
        return f"Target {target} **undifind**"
    return f"Target {target} **undifind**"

# 4) Dijkstra Complexity (Heap)
def dijkstra_time_complexity(v: int, e: int) -> str:
    if v <= 0:
        return "**Time Complexity** = N/A"
    time = (v + e) * math.log2(v)
    return f"**Time Complexity**  ≈ {time:.2f}"

def dijkstra_space_complexity(v: int, e: int) -> str:
    if v <= 0:
        return "**Space Complexity** = N/A"
    space = v + e
    return f"**Space Complexity** = {space}"

# 5) Kruskal Complexity (Heap)
def kruskal_time_complexity(v: int, e: int) -> str:
    time = e * math.log2(v) if v > 0 else 0
    return f"**Time Complexity** ≈ {time:.2f}" if v > 0 else "**Time Complexity** = N/A"

def kruskal_space_complexity(v: int, e: int) -> str:
    space = v + e
    return f"**Space Complexity** = {space}"
